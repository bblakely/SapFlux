...
flux.wcr.interp<-data.frame(matrix(data=-9999,nrow=nrow(flux.wcr), ncol=ncol(flux.wcr)))
colnames(flux.wcr.interp)<-colnames(flux.wcr)
flux.wcr.interp[flux.wcr.interp==-9999]<-NA
#Vars for plotting
interp.spacemap<-data.frame(matrix(data=NA,nrow=nrow(flux.wcr), ncol=ncol(flux.wcr)))
#Reusable params
gaplength<-4  #max number of consecutuve missing values to interpolate across

for(i in 1:14){
  sens<-flux.wcr[,i]  #Pull sensor to fill
...
par(mfrow=c(2,2))
par(xpd=FALSE)
bestfits<-matrix(0,4,14)
DAT.WCR<-flux.wcr.interp  #Use interpolated values

insite.fillmap<-data.frame(matrix(data=NA,nrow=nrow(flux.wcr), ncol=ncol(flux.wcr))) #Create to look at insite filling later

#2.1 Gather information about interpolation
for (s in 1:14){
  
  sens.test<-DAT.WCR[,s]
  fitvec<-rep(0,ncol(DAT.WCR)) #R2
  slopevec<-rep(0,ncol(DAT.WCR)) #Slope
  intvec<-rep(0,ncol(DAT.WCR)) #Intercept
  
  for (i in 1:(ncol(DAT.WCR))){ 
    
    resid<-sens.test-DAT.WCR[,i] #Next several lines remove crazy outliers
    th.h<-quantile(resid, 0.99, na.rm=TRUE)
    th.l<-quantile(resid, 0.01, na.rm=TRUE)
    sens.test[resid>th.h | resid < th.l]<-NaN
    
    plot(sens.test~DAT.WCR[,i], pch='.', main=paste(s,'vs', i)) #Scatterplot
    fit<-lm(sens.test~DAT.WCR[,i]) #Fit regression across sensors and store parameters
    params<-unname(fit[[1]])
    slopevec[i]<-params[2]
    intvec[i]<-params[1]
...
#2.2 Actual filling
flux.wcr.gf<-matrix(NA,nrow(flux.wcr), 14) #Will hold final series
flux.wcr.fills<-matrix(NA,nrow(flux.wcr), 14) #Will hold just gapfilled parts, mainly for plotting

for(i in 1:14){
  flux.gap<-flux.wcr.interp[,i] #pull out sensor of interest
  pair.ind<-bestfits[4,i] #Which sensor is best match?
  flux.pair<-flux.wcr.interp[,pair.ind] #Pull out matching sensor
  ...
  flux.wcr.gf[,i]<-flux.gf #enter into dataframe
